...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'WHATSAPP_READY') {
    ...
    if (dashboardTabId) {
      chrome.tabs.sendMessage(dashboardTabId, { type: 'WHATSAPP_CONNECTED' });
    }
    sendResponse({ success: true });
    return true;
  }
  ...
  return new Promise((resolve) => {
    const listener = (tabId, changeInfo) => {
      if (tabId === whatsappTabId && changeInfo.status === 'complete') {
        ...
        setTimeout(() => resolve(whatsappTabId), 2000); // Aguarda mais um pouco para o WhatsApp carregar
      }
    };
...
chrome.tabs.onRemoved.addListener((tabId) => {
  if (tabId === whatsappTabId) {
    whatsappTabId = null;
    if (dashboardTabId) {
      chrome.tabs.sendMessage(dashboardTabId, { type: 'WHATSAPP_DISCONNECTED' });
    }
  }
