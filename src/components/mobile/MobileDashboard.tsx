...
export function MobileDashboard() {
  ...
  return (
    <div className="p-4 space-y-5 bg-background min-h-full">
      ...
      {isRealAdmin && (
        <div className="relative overflow-hidden rounded-3xl gradient-success p-5 text-primary-foreground shadow-xl glow-success">
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-1 mt-2">
              <div className="flex items-center gap-1 px-2 py-0.5 bg-primary-foreground/20 rounded-full text-xs">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-bold text-success">{stats.pixPago}</p>
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Pagos</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-semibold text-warning">{stats.pixGerado}</p>
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-bold text-success">{stats.boletosPagos}</p>
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Pagos</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-semibold text-warning">{stats.boletosGerados}</p>
        ...
        {/* Cartão */}
        <div className="bg-card border border-border/50 rounded-2xl p-4 space-y-3 col-span-2">
          <div className="flex items-center gap-2">
            <div className="h-10 w-10 rounded-xl bg-info/10 flex items-center justify-center">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div>
                <p className="text-2xl font-bold text-success">{stats.cartaoPago}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Aprovados</p>
              </div>
              <div>
                <p className="text-lg font-semibold text-warning">{stats.cartaoGerado}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Pendentes</p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-2">
          {todayTransactions.slice(0, 5).map((t) => (
            <div 
              key={t.id}
              className={cn(
                "flex items-center justify-between p-3 bg-card border-l-2 rounded-xl",
                t.status === "pago" ? "border-l-success" : "border-l-warning"
              )}
            >
              ...
              <p className={cn(
                "text-sm font-bold",
                t.status === "pago" ? "text-success" : "text-warning"
              )}>
